...
| 15 | Evidensbaseret AI-valg og laering (routing) | **Mission Platform** (Peters egen lagbeskrivelse naevner "participant registry, routing, quotas, budget, mandate, autonomy" — noejagtig SAMARBEJDSMODEL §15's ordforraad) | Forbruger, ikke bygger — ingen AI-routing-motor i TimeLapse | **JA — dette er kernen af Mission Platform-laget per definition** | Selektionsprincipperne (komplementaere kompetencer, laering af udfald) hoerer her |
...
Kortlaegning af Peters efterspurgte 24 begreber mod eksisterende GRC-skema. **REUSE/EXTEND/NEW klassificeret konservativt — NEW kun hvor intet genbrugeligt findes:**
...
| AI/model/version-identitet | **NEW** | Intet fundet noget sted (§1.9) |
| AI opgave-egnethed/performance-evidens | **NEW** | Intet fundet — dette hoerer til Mission Platform (§2) |
| AI-autoritet/autonomi | **NEW, men UPSTREAM** | Samme — hoerer til Mission Platform/Collaborative Intelligence, ikke en TimeLapse GRC-kolonne |
| Routing-beslutning/begrundelse | **NEW, men UPSTREAM** | Samme |

**Konklusion:** af 24 begreber er 6 rene REUSE, 10 EXTEND (af eksisterende TimeLapse-strukturer), 4 NEW-men-lokalt-relevante (compliance-status, applicability, effective-date/fremtidigt-krav, GRC-niveau-override — disse HOERER i TimeLapse), og 4 er NEW-men-UPSTREAM (AI-identitet/performance/autoritet/routing — disse boer IKKE bygges i TimeLapse's GRC-skema).
...
**UPSTREAM.** Bekraeftet nul eksisterende struktur (§1.9). Peters princip 14 (menneske/AI-autoritet) og 15 (evidensbaseret AI-routing) beskriver noejagtigt det SAMARBEJDSMODEL §15 allerede (som Proposed, ikke adopteret) forsoeger at daekke, og det Peter selv definerer som "Mission Platform"-lagets kerne ("participant registry, routing, quotas, budget, mandate, autonomy"). At bygge dette lokalt i TimeLapse's GRC-skema ville skabe en TimeLapse-laast kopi af noget der eksplicit er tvaergaaende. **Anbefalet retning (DECISION REQUIRED, ikke udfoert):** faerdiggoer/adoptér SAMARBEJDSMODEL §15 (eller den bredere Mission Platform/Collaborative Intelligence-arkitektur den peger paa) FOERST, og lad TimeLapse's GRC-skema blot faa et TYNDT REFERENCE-felt (`owner_type: human|ai`, `owner_authority_ref`) der peger UD til den kommende struktur, i stedet for at TimeLapse selv definerer autoritetsniveauer.
...
- **Loebende, risikobaseret overvaagning/alarmering (§16c, valgfri):** **NEW, genuinely missing**. Bekraeftet nul flaade-niveau flapping-/restart-loop-/health_gate-detektion. Edge1-pydantic-haendelsens 13.039 genstarter over 19 timer uden alarm er det konkrete, evidensbaserede bevis for dette hul (allerede dokumenteret i tidligere PR-revision).
...
- Flaade-niveau risikobaseret overvaagning/alarmering (restart-loop/flapping-detektion) — findes IKKE.
...
- **AI-provider-/model-routing med evidensbaseret tillid** (princip 15) — hoerer i Mission Platform. TimeLapse boer forbruge en faerdig model, ikke bygge sin egen parallelle.
