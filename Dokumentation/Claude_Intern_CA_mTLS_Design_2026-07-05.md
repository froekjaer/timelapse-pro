...
| 2026-07-06 (kodefase, del 1) | Claude: §9 trin 3's forudsætning delvist forberedt — `KeyCredential` (headend/database.py) udvidet med `key_type="mtls_device_cert"` som dokumenteret, gyldig fremtidig værdi (GENBRUG af den eksisterende credential-lifecycle-tabel, ikke en ny parallel tabel — se modellens docstring for hvorfor denne genbrug giver mening, i modsætning til #62's AccessTicket). SKEMA/MODEL-KODE ALENE: ingen CSR-signerings-endpoint, ingen CA-nøgle, ingen Root/Issuing CA-generering. `metadata_json`-kontrakten for dette key_type (serial_number, not_before, issuer_cn, lifetime_days_at_issuance, expiry_grace_policy_at_issuance) er dokumenteret som forberedelse til den fremtidige endpoint. Testet (7 pytest-tests + fuld suite, 78/78 grønt), committet via `claude_proxy.py`. §9 trin 2 (Root/Issuing CA-generering, Peters eget terminal) og trin 3's faktiske endpoint-kode er fortsat ikke bygget. |
