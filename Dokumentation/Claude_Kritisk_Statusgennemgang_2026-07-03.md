...
| 16 | Kryds-kunde-lækage af billeddata ved Edge-gentildeling (R16, fundet under fase 3-implementering) | 🔴 P0 | ✅ Rettet i kode 2026-07-03, committet (`bb18421`) og pushet. Se fund #4 ovenfor og `RISK_ASSESSMENT_v10.md` R16. Konkret, udnyttelig lækage — ikke kun teoretisk — da en fysisk Edge-enhed reelt kan genbruges på tværs af kunder. Afventer live-verifikation efter genstart | Claude (kode+test gjort), Peter (deploy) |
| 17 | `Capture` mangler `site_id` — hierarkiet kunde/site/kamera-lokation var ikke fuldt tagget på billedet (fase 4, rejst af Peter 2026-07-03) | 🟢 P3 | ✅ Rettet: skema v13 + resolver udvidet til 3-tuple + backfill-script udvidet + additivt i `/api/admin/captures`-response, verificeret (TestClient). Bevidst KUN tagging — ingen håndhævelse endnu, kræver separat beslutning om at udvide `User`-modellen med site-scope. Afventer commit | Claude (kode+test gjort), Peter (deploy + evt. fremtidig beslutning om håndhævelse) |
