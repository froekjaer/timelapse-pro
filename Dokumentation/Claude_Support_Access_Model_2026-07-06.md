...
| 2026-07-06 (kodefase, del 1) | Claude: byggede den anbefalede `AccessTicket`-model i `headend/database.py` (ny, separat tabel, jf. §6) — SKEMA/MODEL-KODE ALENE, testet (7 pytest-tests + fuld eksisterende suite, 78/78 grønt), committet via `claude_proxy.py`. Fortsat INGEN udstedelses-endpoint, script eller Support-CA-nøgle — §8's punktliste er uændret gyldig bortset fra selve tabellens eksistens. `credential_id`-feltet er en fremadrettet, ikke-brugt kobling til en fremtidig `KeyCredential`-række for selve SSH-certifikatet. |
