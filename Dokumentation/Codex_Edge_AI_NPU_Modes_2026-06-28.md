...
## NPU-runtime status 2026-07-02

ACUITY/VIPLite-kæden er nu bevist end-to-end på Orange Pi 4 Pro:

- `.nb` genereret fra MobileNet smoke-modellen via ACUITY Docker `ubuntu-npu:v2.0.10.2`.
- Installeret på `timelapse0101` som `/opt/timelapse/models/edge_qa.nb`.
- VIPLite-wrapperen kører modellen på `/dev/vipcore`.
- `edge_qa_npu_runner.py` returnerer `timelapse.edge_qa.v1` med `available=true`.
- Selve NPU-kørslen er hurtig: typisk omkring 2-3 ms for `vip_run_network` på testbilledet.

Teknisk fix i wrapperen:

- NBG-inputbufferen er FP16 og 301.056 bytes (`224*224*3*2`), ikke `uint8`.
- Wrapperen laver nu FP16 input med konfigurerbar skala via `TIMELAPSE_EDGE_QA_INPUT_SCALE`
  (default `1/255`).
- Vendor-output parseren håndterer støjende stdout/stderr mere robust.

Driftsstatus: `assist`, ikke autonomt. Runtime virker, men den kvantiserede `.nb` skal kalibreres
bedre, fordi scorefordelingen endnu ikke matcher ONNX CPU-baseline tæt nok. Næste runde bør være en
systematisk ONNX-vs-NPU eval på historiske Travbyen/Nordre Villavej-billeder og derefter en ny
ACUITY-export med forbedret calibration/inputmeta.
