...
| C-03 | Standard super_admin-password er ændret fra default | 🔴 Bekræft manuelt — **Delvist fremskridt 2026-07-06 (periodisk tjek #85):** ny `_warn_if_default_admin_password_active()` (headend/main.py) kører ved HVERT opstart (ikke kun ved oprettelse) og logger `log.critical(...)` (fanges af SIEM via den generiske log-pipeline) hvis en aktiv admin/super_admin-konto stadig autentificerer mod "changeme". Dette giver vedvarende, automatisk SIEM-synlighed af risikoen — men erstatter IKKE selve den manuelle bekræftelse på de faktiske rd/staging/prod-maskiner, som stadig udestår (ingen shell-adgang herfra). Test: `headend/tests/test_default_admin_password_warning.py` (6 nye tests, ikke kørt — se HANDOVER_LOG, `mcp__workspace__bash` stadig nede) |
...
| M-05 | Agent/service-principal-model (Codex-forslag) med hård prod-afvisning implementeret | 🔴 Designdiskussion i gang (se `HANDOVER_LOG.md` 2026-07-05), intet kodet — nu en teknisk defense-in-depth-håndhævelse af en allerede besluttet politik (M-02), ikke selve beslutningen. **Forudsætning (2026-07-06, periodisk tjek #65):** ret `TIMELAPSE_ENV`-terminologi-drift (`"rd"` mangler i 3 eksisterende kodesteder, se `RISK_ASSESSMENT_v10.md` R19) FØR denne håndhævelse bygges. **Forudsætning rettet (2026-07-06, periodisk tjek #84):** `"rd"` tilføjet i `edge/agent.py` + `headend/siem.py` (`headend/main.py` krævede ingen ændring). Selve M-05-håndhævelsen (AgentPrincipal-middleware) er stadig ikke bygget — kun blokerende faldgrube ryddet på forhånd |
...
| M-08 | Kontrolleret, logget break-glass support-adgang (installation/fejlsøgning) — design | 🟠 Design-notat klar (`Claude_Support_Access_Model_2026-07-06.md`), INGEN kode/CA/script bygget endnu. Nøgleelementer: separat Support-CA (ikke device-CA'en fra #52), korttidslevende SSH-certifikater med indbygget kryptografisk udløb, kunde-samtykke-tjek pr. aktivering, signeret ticket + audit-log. Peter er eneste, der kan aktivere adgangen |
...
| M. Miljøadskillelse/agent-adgang | 1 blokker (M-05: teknisk håndhævelse/defense-in-depth af nul-agent-adgang ikke implementeret, R19) — M-02 (selve politikken) er ✅ besluttet/bekræftet af Peter 2026-07-05 | 🔴 Ikke klar |
