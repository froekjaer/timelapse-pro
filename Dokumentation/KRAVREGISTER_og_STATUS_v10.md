...
| UPD-012 | Per-target deployment status | 🟠 Regression i deployet fix 2026-07-05 | `update_targets`-tabel + `/api/updates/{id}/flow-status` + UI fandtes allerede (siden juni); rettelsen for global `PendingUpdate.status` (`61802951`, deployet) manglede en `db.flush()`, så multi-target rollouts aldrig flippede til deployed/rolled_back (fundet via ny kontrakttest `headend/tests/test_report_update_rollup.py`). Ny 1-linjes rettelse testverificeret (4/4), IKKE committet/deployet endnu — se GO_LIVE_CHECKLIST §K, RISK_ASSESSMENT R06 |
