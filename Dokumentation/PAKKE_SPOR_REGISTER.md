...
- **Base/head:** PR #258, draft mod `main`. Senest verificeret 2026-09-25: base `b901d486733d69733bad2f21dfa42354573b6aba`; docs-head `81d76f1ee4754f86f424a20721222f84277ed9b0`, seneste kode/test-head `592b2cd1671348b955170e2038efa047f767ea96`. GitHub rapporterede mergeable=true før docs-opdateringen. SHA'er er snapshots og skal genhentes før merge.
...
- **Seneste verificerede aktivitet/evidens:** TRAVBYEN-001 er menneskereviewet for privacy. Fysisk benchmark v3 på head `6c610f59` brugte live approved DB-vocabulary (37 kategorier / 664 tags, ingen fallback) og bekræftede unknown-tag promotion fysisk: fem ukendte Apple-observationer blev bevaret som unapproved `new_tags`/adapter provenance. Apple fandt personforekomst korrekt; Ollama missede den. Efterfølgende hardening på kode-head `592b2cd1` fjerner forventet cv2-warning uden OpenCV-dependency, bevarer ægte raw Apple response separat fra adapter response og gør tag-translation provenance provider-aware. CI run `36115141110` var in_progress ved registeropdateringen; denne head må derfor ikke omtales som grøn endnu. Det er case-evidence, ikke provider-ranking.
- **Næste handling:** Verificér CI på provenance/cv2-head og lav én kort fysisk regression: ingen `No module named 'cv2'` warning, raw provider-response adskilt fra `adapter_response`, unknown-tag promotion fortsat intakt. Derefter udvid til 10–20 repræsentative, menneskereviewede captures med live database vocabulary og tilføj Gemini via autoritativ Headend-konfiguration. Ingen provider-defaults/prompttuning før repræsentativt ground truth.
