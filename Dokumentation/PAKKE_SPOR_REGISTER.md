...
- **Seneste verificerede aktivitet/evidens:** Capability architecture baseline er grøn i CI. Fysisk capability smoke på Mac Mini gav Apple Text+Structured PASS og Ollama Text+Structured PASS. Gemini Text+Structured fejlede på legacy region/model mismatch: `gemini-3.8-flash` blev bygget med `europe-west1`. Google understøtter 3.8 Flash på Vertex i `global/us/eu`; kode-head `370ae29a` gør location model-aware (`europe-*` -> `eu`, `us-*` -> `us` uden global widening), tilføjer separat `gemini_text_location=eu`, reel remote availability-probe og secret-safe failure diagnostics. CI på denne sidste kode-head var pending ved opdateringen.
- **Arkitekturstatus:** **Code-complete for diagrammets Vision/Text/Structured scope.** Ikke production-accepted endnu.
- **Åbne acceptance-gates:** Apple/Ollama Text+Structured smoke er fysisk PASS. Gemini skal gen-smokes efter region-fix og grøn CI. Derefter: 10–20 reviewede image cases; RAM/concurrency/thermal/fallback evidence; idempotent GRC-import kørt og verificeret i autoritativ Headend DB. Ingen production-default ændres før disse gates er accepteret.
...
- **Næste handling:** Verificér CI på Gemini region/diagnostic-head og genkør capability smoke for Apple/Ollama/Gemini. Derefter corpus/resource/GRC gates før merge/defaultændring.
