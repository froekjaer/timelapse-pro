...
- **Seneste verificerede aktivitet/evidens:** Capability-contract, Apple/Ollama/Gemini adapters og router er implementeret. Image worker/manual/review/backfill/batch routes via provider-laget. Natural Search, AI Ops, AI-assisteret SIEM og CMDB bruger generic Text/Structured. Deterministic SIEM er uafhængig. AI management har provider order/status. Fysisk Travbyen-benchmark har verificeret Apple vision, live DB-vocabulary, privacy normalization, raw-vs-adapter provenance, cv2/PIL fallback og unknown-tag preservation. Gemini probe blev hardenet til reel `health_check()`; kode-head `c3229e0a2d95cfd4bba01af1e0e4adaca88bfeeb` er verificeret grøn i GitHub Actions run `36121484729`.
- **Arkitekturstatus:** **Code-complete for diagrammets Vision/Text/Structured scope.** Ikke production-accepted endnu.
- **Åbne acceptance-gates:** fysisk Text+Structured capability-smoke for konfigurerede Apple/Ollama/Gemini; 10–20 reviewede image cases; RAM/concurrency/thermal/fallback evidence; idempotent GRC-import kørt og verificeret i autoritativ Headend DB. Ingen production-default ændres før disse gates er accepteret.
...
- **Næste handling:** Kør provider capability smoke på Headend. Derefter corpus/resource/GRC gates før merge/defaultændring.
