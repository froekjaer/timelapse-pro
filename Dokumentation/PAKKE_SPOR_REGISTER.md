...
- **Seneste verificerede aktivitet/evidens:** Capability architecture og Gemini region/diagnostic hardening er grøn i CI (kode-head `370ae29a`, run `36122688710`). Fysisk Mac Mini smoke gennem CapabilityRouter er nu komplet: Apple Text+Structured PASS, Ollama Text+Structured PASS og Gemini 3.8 Flash Text+Structured PASS på Vertex `eu`, samlet `all_ok=true`. Tidligere Gemini model/region mismatch er dermed fysisk verificeret lukket. Travbyen-sporet har desuden fysisk verificeret Apple/Ollama Vision, live vocabulary, privacy normalization og provider/adapter provenance. Dette er capability/runtime-evidence, ikke en semantisk provider-ranking.
...
- **Næste handling:** Kør og verificér AI-provider GRC-import på Headend. Derefter corpus/resource gates før merge/defaultændring.
