...
- **Formål/scope:** (a) Edge API-mTLS-enrollment (øvrige commits); (b) dedikeret sshd-instance på 22022 til Edge-reverse-tunnels: `timelapse_tunnel`-servicekonto (pubkey-only, ingen shell/PTY, remote-forward alene), pr.-device `restrict,port-forwarding,permitlisten="127.0.0.1:2201|2204"` authorized_keys (`restrict` slår al forwarding fra; `port-forwarding` genaktiverer den; `permitlisten` indsnævrer til én loopback-port), `GatewayPorts no` (loopback-binding — erstatter dagens wildcard-bindende `Match User tunnel`/`GatewayPorts yes`-sti ved cutover), dedikeret hostnøgle pr. instance til Edge-pinning, launchd LaunchDaemon med `sshd -t`-validering før aktivering.
...
- **Næste handling:** (1) Peter kører installer med sudo (`--self-test`; rettelser efter eksternt review `ea8c4826`→fix-commit 2026-09-17: korrekt `restrict,port-forwarding,permitlisten`-syntaks, ssh-options før destination, `--verify-only` beviseligt read-only, deterministisk self-test-fejlrapportering + `--uninstall`; regressions-tests `tests/test_timelapse_tunnel_sshd_config.py`), (2) NAT/firewall 22022, (3) Edge-cutover-fase: per-device nøgler + `permitlisten`-entries + hostnøgle-pinning + `ssh_manager`-endpointskifte til `timelapse_tunnel@backend.timelapse-pro.dk:22022`, (4) derefter oprydning af legacy `Match User tunnel`-blok (særskilt beslutning).
