...
- **Seneste verificerede aktivitet/evidens:** Fysisk benchmark v3 på head `91d6bfea` verificerede live DB-vocabulary (37/664, ingen fallback), ingen cv2-warning, raw-vs-adapter provenance og unknown-tag promotion. Samme run afslørede den modsatte bucket-fejl: fire allerede-approved canonical tags blev placeret af Apple i `new_tags` og forsvandt i canonical output. Kode-head `6bfa0096` reklassificerer nu begge veje mod TimeLapse vocabulary authority og har regressionstests; CI run `36118641188` var queued ved opdateringen. Forrige provenance/cv2 head `592b2cd1` er grøn i run `36115141110`. Det er fortsat case-evidence, ikke provider-ranking.
- **Næste handling:** Verificér CI på bidirectional tag-reclassification-head og lav én sidste TRAVBYEN-001 regression: known-in-`new_tags` skal lande i canonical `approved_tags`, unknown-in-`tags` skal fortsat lande i `new_tags`. Derefter udvid til 10–20 repræsentative, menneskereviewede captures med live database vocabulary og tilføj Gemini via autoritativ Headend-konfiguration. Ingen provider-defaults/prompttuning før repræsentativt ground truth.
