...
- **Aktivt integration-/closure-spor 2026-09-25:** branch `chatgpt/api-mtls-runtime-closure-20260925` er reconcileret mod frisk `main@b901d486733d69733bad2f21dfa42354573b6aba` som fortsættelsen af #246/#259. Reverse-SSH sub-track B er allerede fysisk E2E-verificeret; aktiv kodeændring vedrører mTLS runtime-closure samt replay/MFA-reviewfund. Ingen fysisk CA-ceremoni, Edge enrollment, nginx enforcement eller legacy-retirement er autoriseret af denne kode-merge. Uafhængigt Claude/Z.ai-review og CI på main-baseret PR kræves før fysisk aktivering; Peter ejer merge/aktivering/retirement. Cross-repo/website-påvirkning: **no impact**.
...
- **Berørte domæner/kontrakter:** Headend tunnel-ingress (`deploy/ssh/*`, launchd, portkontrakter), Edge tunnel manager `edge/tunnel/ssh_manager.py`, Edge systemd unit `edge/scripts/timelapse-edge.service`, relevante regressionstests samt mTLS foundation-filer. Admin SSH/SFTP 22222 og nginx/API 8443 er ikke erstattet af reverse-SSH-sporet.
- **Base/head:** PR #246 (draft), branch `chatgpt/api-mtls-20260917`, base `main` (`928134be847eb96df25455736f6acd766aff84f2` ved PR-oprettelse), seneste runtime-verificerede kode-head **`966429a711b1662f596f50d04c4a1ffc845e7db0`**.
- **Sub-track B — reverse-SSH status:** **FYSISK E2E-VERIFICERET.** Dedikeret sshd på Headend TCP/22022 er installeret og funktionelt verificeret; offentlig DNS/NAT til `tunnel.timelapse-pro.dk:22022` er verificeret mod dedikeret host-key fingerprint `SHA256:TwExDmMYpnXX+qK/ZrLwpN2rb8bQiP/FLHKOkfTdbP4`. Edge1 bruger `127.0.0.1:2201`; Edge2 bruger `127.0.0.1:2204`. Begge Edges er cuttet over til `timelapse_tunnel@tunnel.timelapse-pro.dk:22022` med per-device nøgle og explicit loopback `-R`.
- **Edge2 systemd-remediation:** fysisk restart afdækkede latent EROFS-fejl ved manglende `/var/log.hdd/timelapse` under `ProtectSystem=strict`. Backing filesystem var RW. Commit `966429a7` ændrer prestart til `ExecStartPre=+/bin/mkdir -p /var/log.hdd/timelapse/breakglass/sessions`, mens `ProtectSystem=strict` og `ReadWritePaths=-/var/log.hdd/timelapse` bevares. Fysisk systemd-249 probe og efterfølgende kontrolleret Edge2 restart bestod. Installeret Edge2-unit SHA256: `9603e768b36fdd2d6b0e39658683e34f1135c778972bd68cba6eddfeac6f00f2`; rollback-backup bevaret.
- **E2E-evidens Edge2:** Headend SSH via `127.0.0.1:2204` returnerede hostname `tl-modbaggarddlvc`, user `orangepi`, `timelapse-edge.service=active` og den installerede `ExecStartPre=+...`-linje. Ingen nye `Read-only file system`-advarsler efter fixed restart.
- **CI/test:** targeted regression 53/53 passed lokalt. TimeLapse Pro CI run #1099 **SUCCESS** på eksakt head `966429a711b1662f596f50d04c4a1ffc845e7db0`.
- **Sub-track A — mTLS status:** foundation er implementeret i repo, men runtime wiring/enrollment orchestration, HeadendClient cert-brug, proxy/enforcement, migration/lifecycle og fysisk deployment er fortsat uafsluttet. PR #246 skal derfor fortsat være DRAFT og må ikke merges som om hele sikkerhedssporet er færdigt.
- **Overlap/restdisposition:** Reverse-SSH sub-track B er fysisk færdigverificeret, men beholdes i samme draft-PR som det endnu uafsluttede mTLS sub-track A. Legacy tunnelkonfiguration, gamle nøgler og rollback-backups fjernes ikke i dette spor endnu. Det separate `timelapse-breakglass-setup.service`-fund er registreret, men ikke blandet ind i denne remediation.
- **Cross-repo/website-påvirkning (§16.10-felt):** Ingen påvirkning fundet; ændringerne er TimeLapse-lokal sikkerheds-/deploy-infrastruktur og Edge-runtime.
- **Seneste verificerede aktivitet/evidens:** 2026-09-17 fysisk Edge1/Edge2 cutover, Edge2 service-remediation, E2E SSH gennem Headend-loopback og CI #1099 SUCCESS.
- **Næste handling:** fortsæt Sub-track A mTLS-runtimearbejdet. Legacy reverse-tunnelsti, gamle nøgler og backups beholdes som rollback indtil særskilt cleanup-/retirement-beslutning. Separat systemd-warning i `timelapse-breakglass-setup.service` (`StartLimitIntervalSec` i forkert sektion) håndteres som eget remediation-fund.
- **Blokeringsansvarlig:** mTLS-implementering fortsætter på #246; Peter ejer merge- og legacy-retirement-beslutning.
- **Opfølgning:** Før merge af #246 samt før enhver retirement/sletning af legacy reverse-tunnelsti, gamle Edge-nøgler eller rollback-backups.
