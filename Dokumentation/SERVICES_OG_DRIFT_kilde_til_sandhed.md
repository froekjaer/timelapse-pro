...
Opdateres når noget ændres. Sidst rørt: 2026-06-28.
...
|---|---|---|---|
| Headend (FastAPI/uvicorn) | launchd user-agent `dk.froekjaer.timelapse-headend` | `127.0.0.1:8000` | Repo `~/projects/timelapse-pro`, venv `~/.venvs/timelapse-headend` |
| PostgreSQL | lokal | `localhost:5432` | DB `timelapse_db`, brugere `timelapse` / `peter` |
| Ollama (lokal vision) | lokal | `127.0.0.1:11434` | Model `qwen2.5vl:7b` |
| nginx | foran headend | `80/443` (mål: Cloudflare Tunnel) | Serverer UI, proxy `/api` → `127.0.0.1:8000` |
...
launchctl print gui/$(id -u)/dk.froekjaer.timelapse-headend | grep -E "state = |pid = |last exit"
```
