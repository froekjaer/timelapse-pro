...
class QualityChecker:
    ...
    def _cv_features(self, filepath: Path) -> dict:
        ...
        try:
            ...
            tile_laps = []
            low_texture_tiles = 0
            low_saturation_low_texture_tiles = 0
            grid_h, grid_w = 8, 8
            tile_h = max(1, gray.shape[0] // grid_h)
            tile_w = max(1, gray.shape[1] // grid_w)
            for y in range(grid_h):
                for x in range(grid_w):
                    tile = gray[y * tile_h:(y + 1) * tile_h, x * tile_w:(x + 1) * tile_w]
                    sat_tile = hsv[y * tile_h:(y + 1) * tile_h, x * tile_w:(x + 1) * tile_w, 1]
                    if not tile.size:
                        continue
                    tile_lap = float(cv2.Laplacian(tile, cv2.CV_64F).var())
                    tile_sat = float(np.mean(sat_tile)) if sat_tile.size else saturation_mean
                    tile_laps.append(tile_lap)
                    if tile_lap < 30:
                        low_texture_tiles += 1
    ...
    def _classify_scene_issue(self, quality: QualityResult, features: dict) -> dict | None:
        ...
        low_texture_tiles = float(features.get("low_texture_tile_ratio", 0))
        low_sat_low_texture_tiles = float(features.get("low_saturation_low_texture_tile_ratio", 0))

        if (
            self._lens_obstruction_enabled
            and blur < self._blur_thresh
            and saturation < 35
            and low_sat_low_texture_tiles > 0.75
        ):
            return {
                "is_anomaly": True,
                "probable_cause": "snow_or_dirt_on_lens",
                "confidence": 0.88,
                "recommended_action": "Kontroller frontglas/linse for sne, is, skidt eller saltfilm.",
            }
    ...
    def _merge_npu_result(self, report: dict, npu_result: dict) -> dict:
        ...
        if npu_result.get("accepted") is False:
            return report
