...
#undef ALOGE
#undef ALOGW
static int awnn_log_enabled(void) {
    const char *value = getenv("TIMELAPSE_EDGE_QA_VIPLITE_DEBUG");
    return value != NULL && strcmp(value, "0") != 0;
}
#define ALOGD(...) do { if (awnn_log_enabled()) fprintf(stderr, __VA_ARGS__); } while (0)
#define ALOGE(...) fprintf(stderr, __VA_ARGS__)
#define ALOGW(...) do { if (awnn_log_enabled()) fprintf(stderr, __VA_ARGS__); } while (0)
