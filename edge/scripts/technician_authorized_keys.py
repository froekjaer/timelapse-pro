...
The %u is load-bearing: without it sshd never passes the requested username
as argv[1], so the guard below never matches and no key is ever served to
anyone. Confirmed live 2026-08-24 — see edge/agent.py's
_repair_sshd_authorized_keys_command_missing_u_token() for the self-heal
this shipped with for already-provisioned devices.
