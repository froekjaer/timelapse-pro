...
def _normalise_vendor_payload(payload: dict[str, Any], model: Path | None, runtime_info: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("input_layout"):
        input_dtype = str(payload.get("input_dtype") or "fp16")
        contract["model_input"] = {
            "width": 224,
            "height": 224,
            "channels": 3,
            "layout": str(payload["input_layout"]),
            "color_order": "RGB" if "rgb" in str(payload["input_layout"]).lower() else "BGR",
            "scale": float(os.getenv("TIMELAPSE_EDGE_QA_INPUT_SCALE", str(1.0 / 255.0))),
            "dtype": input_dtype,
        }
