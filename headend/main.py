...
# TIMELAPSE_ENV, JWT_SECRET, JWT_ALGORITHM, JWT_EXPIRE_H, COOKIE_NAME, COOKIE_SECURE
# moved to auth.py (2026-08-26) — imported below, near the other extracted-module
# imports, alongside the rest of the auth/RBAC kernel.
...
#Peter        from sqlalchemy import text
    ...
    try:
        ...
        start_ai_batch_background_loop(interval_m)
...
# Auth/session/RBAC kernel (_hash_password .. require_role) moved to auth.py
# (2026-08-26) — imported near the top of this file, alongside the other
# extracted-module imports. See headend/auth.py's module docstring for why.
...
# ═══════════════════════════════════════════════════════════════════════════
# list_cameras + download_camera_ssh_key (legacy stub) moved to
# headend/api/cameras_api.py (2026-08-27, Phase 2 of the main.py
# modularization plan), along with the rest of the Camera domain further
# down. GET /api/admin/headend/ssh-public-key right below is NOT part of
# that domain (headend's own SSH keypair) and stays here unchanged.
...
# Camera CRUD, BT-TOTP provisioning, and assignment lifecycle (create_camera
# .. camera_assignment_history) moved to headend/api/cameras_api.py
# (2026-08-27, Phase 2 of the main.py modularization plan) — imported near
# the top of this file, mounted below.
...
def _run_refresh_catalog_job(job_id: str, payload_data: dict, username: str) -> None:
    ...
    try:
        ...
        image = payload.image or _default_ubuntu_image_for_device(inv)
        ...
        result = import_os_catalog_from_lab_apt_list(
            OsCatalogImportPayload(
                ...
                source=payload.source or f"mac-docker-builder:{image}:{device_id}",
...
def _generate_os_update_catalog_candidates(
    *,
    ...
    try:
        ...
        image_version = str(image or "").rsplit(":", 1)[-1].strip()
        generated = _generate_apt_list_from_ubuntu_metadata(
            installed=installed,
            device_id=device_id,
            architecture=architecture,
            suite=_ubuntu_suite_for_version_label(image_version),
        )
        ...
        for inv in devices:
            ...
            try:
                generated = _generate_os_update_catalog_candidates(
                    ...
                    device_id=device_id,
                    architecture="arm64",
                    image=_default_ubuntu_image_for_device(inv),
                )
...
def _ubuntu_suite_for_version_label(label: str) -> str:
    return {
        "24.04": "noble",
        "22.04": "jammy",
        "20.04": "focal",
        "18.04": "bionic",
    }.get(str(label or "").strip(), "noble")


def _default_ubuntu_image_for_device(inv: DeviceInventory | None) -> str:
    """Docker image tag matching this device's own reported Ubuntu release,
    so the OS-catalog builder compares against the right package repository
    instead of silently defaulting to noble/24.04 for every device.

    Found 2026-09-09: the scheduled cmdb-refresh job hardcoded "ubuntu:24.04"
    for every apt-managed device, so Edge 2 (genuinely Ubuntu 22.04.5/jammy,
    confirmed via /etc/os-release) had its OS-security-update candidates
    generated by comparing against the noble catalog — wrong package
    versions, and any jammy-specific security update not also present in
    noble would never have been proposed at all.
    """
    suite = _infer_ubuntu_suite_for_os_bundle(inv, [])
    return f"ubuntu:{_ubuntu_version_label_for_suite(suite)}"
...
def report_update(
    ...
    if not u:
        raise HTTPException(status_code=404)
    authorized_target = db.query(UpdateTarget).filter_by(
        pending_update_id=u.id,
        device_id=device_id,
    ).first()
...
# Gemini Batch API (start/list routes + poller thread) moved to
# headend/api/ai_batch_api.py (2026-08-26, Phase 1 of the main.py
# modularization plan) — imported near the top of this file, mounted
# below; poller thread now started via start_ai_batch_background_loop()
# in startup(), same position as the old inline threading.Thread(...) call.
...
# _deep_merge moved to dict_merge.py (2026-08-26, part of the auth.py extraction —
# auth.py's session-policy resolution needed it too) — imported near the top
# of this file alongside the other extracted-module imports.
...
# Edge disk-image build + WiFi-injection pipeline moved to
# headend/api/edge_disk_image_api.py (2026-08-26, Phase 1 of the main.py
