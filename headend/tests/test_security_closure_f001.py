"""F-001 security closure: Headend config merge must not emit shared TOTP fallback."""
import json
import os
import pathlib
import sys
...
def _make_device(session, device_id="TL-F001UNPROVISIONED"):
...
def test_actual_config_merge_has_no_shared_factory_totp_fallback(db_session):
    _make_device(db_session)

    cfg = main.get_config("TL-F001UNPROVISIONED", _auth=None, db=db_session)
...
def test_explicit_camera_totp_recovery_adapter_remains_available(db_session):
    _make_device(db_session, "TL-F001PROVISIONED")
    ...
    cfg = main.get_config("TL-F001PROVISIONED", _auth=None, db=db_session)
