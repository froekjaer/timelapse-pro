...
import { FlaskConical, ArrowLeft, RefreshCw, Thermometer, HardDrive, Wifi, Clock, Image, Settings, Camera, BarChart2, X, ChevronLeft, ChevronRight, Heart, CalendarDays } from 'lucide-react'
    ...
    >
      ...
      <div className="px-2 py-1.5">
        ...
        {capture.blur_score != null && (
          <p className="text-xs text-gray-400 mt-0.5">blur {Math.round(capture.blur_score)}</p>
        )}
        <p className="text-xs text-gray-300 mt-0.5 truncate font-mono" title={capture.filename}>{capture.filename}</p>
      </div>
    </div>
  )
}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              {diagnostics.ssd_free_gb != null && <p className="text-xs text-gray-300 mt-1">{diagnostics.ssd_free_gb.toFixed(1)} GB ledig</p>}
            </div>
            {/* NTP */}
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {tab === 'timeline' && <TimelineNavigator captures={captures} onSelect={i => { setLightbox(i) }} />}
