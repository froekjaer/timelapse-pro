...
def test_sync_captures_in_capture_cycle():
    ...
    content = read_agent()
    # Find capture cycle og verificer sync er der
...
def test_sync_captures_info_logging():
    ...
    content = read_agent()
    assert 'log.info("Syncing' in content or "log.info('Syncing" in content, \
        "FEJL: _sync_captures skal logge på INFO niveau!"

def test_sync_captures_called_at_startup():
    """_sync_captures skal kaldes ved startup."""
    content = read_agent()
    idx_startup = content.find("Running startup sequence")
    idx_sync = content.find("self._sync_captures()", 0, idx_startup + 500)
    assert idx_sync > 0, \
        "FEJL: _sync_captures skal kaldes ved startup!"
