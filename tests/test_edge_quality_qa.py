...
from edge.tools.analyse_qa_batch import iter_images
...
def test_qa_report_detects_snow_or_dirt(monkeypatch):
    ...
    monkeypatch.setattr(checker, "_cv_features", lambda _path: {
        ...
        "low_saturation_low_texture_tile_ratio": 0.95,
    })
...
def test_qa_report_prioritises_full_frame_front_glass_snow_over_sun(monkeypatch):
    checker = make_checker()
    ...
    def fake_check(_path, _expected_sha256=None):
        return QualityResult(
            ...
            passed=False,
            blur_score=12.0,
            brightness_mean=152.0,
            blur_threshold=80.0,
            dark_threshold=25.0,
            bright_threshold=230.0,
            sha256_verified=True,
            message="soft low-saturation winter glare",
        )

    monkeypatch.setattr(checker, "check", fake_check)
    monkeypatch.setattr(checker, "_cv_features", lambda _path: {
        "highlight_ratio": 0.07,
        "center_brightness": 144.0,
        "contrast_std": 51.0,
        "bright_ratio": 0.13,
        "saturation_mean": 16.0,
        "low_saturation_low_texture_tile_ratio": 0.98,
    })

    report = checker.qa_report(Path("travbyen_frontglass_snow.jpg"))

    assert report["probable_cause"] == "snow_or_dirt_on_lens"
    assert report["confidence"] >= 0.85


def test_analyse_batch_skips_headend_thumbnails(tmp_path):
    original = tmp_path / "image.jpg"
    thumb_dir = tmp_path / ".headend-thumbs"
    thumb_dir.mkdir()
    thumb = thumb_dir / "image.jpg"
...
def test_travbyen_realworld_frontglass_snow_fixture_if_available():
    path = Path("/Volumes/data-fast/timelapse-incoming/canonical-images/Kirkbi_A_S/Travbyen/Kamera_2/2026/01/06/Kirkbi_A_S_Travbyen_Kamera_2_20260106_125938.jpg")
    if not path.exists():
        pytest.skip("Travbyen real-world fixture is only available on Peter's headend volume")

    report = make_checker().qa_report(path)
